...
def repeat():
    ...
    while(signals[currentGreen].green > 0):   # while the timer of current green signal is not zero
        ...
        # Check if no vehicles are detected in the current green signal's lanes
        if no_vehicles_detected(currentGreen):
            print("No vehicles detected. Switching to next signal.")
            break  # Exit the loop to switch to the next signal
...
def updateValues():
    for i in range(0, noOfSignals):
        if(i == currentGreen):
            if(currentYellow == 0):
                if not no_vehicles_detected(currentGreen):  # Only decrement if vehicles are present
                    signals[i].green -= 1
            ...
            signals[i].red -= 1
def no_vehicles_detected(currentGreen):
    """
    Check if there are no vehicles in the lanes of the current green signal.
    """
    direction = directionNumbers[currentGreen]
    for lane in range(3):  # Check all 3 lanes
        if len(vehicles[direction][lane]) > 0:
            return False  # Vehicles are present
    return True  # No vehicles detected
